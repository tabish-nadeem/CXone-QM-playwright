...
Given("Step-1: Should Verify Employee Evaluation", { timeout: 60 * 1000 }, async () => {
    ...
    await performanceMonitoring.selectForm('standardEvaluationForm');
    console.log("Selected form");
    let toastmsg = await performanceMonitoring.getToastMessage();
    expect(toastmsg).toContain('Evaluation assigned successfully.');
    await performanceMonitoring.toastClose();
});
...
Then("Step-5: Getting Performance Details of Manager", { timeout: 180 * 1000 }, async () => {
